...
- **Webhook 受信 Lambda は同期で署名検証 → DB INSERT → SQS enqueue → 200**。AI 下書き生成は **Worker Lambda** に分離し、Anthropic API を非同期で呼ぶ。Send API 呼び出しは管理画面の `createServerFn` から同期的に実行（人間トリガ）。
- **DB = Supabase Postgres（東京リージョン、無料プラン）**。Drizzle ORM で型安全アクセス。RLS は採用しない（シングルテナント運用、運営者が自分のデータを見るだけ）。EventBridge keep-alive Lambda で Pause 回避（FR-027）。
- **認証 = Supabase Auth**（メール+パスワード、JWT は HttpOnly Cookie）。Cognito / DB セッションテーブルは持たない。
- **Lambda は VPC 外配置**。NAT Gateway / RDS Proxy / Cognito モジュールは廃止。Supabase / Anthropic / Meta は外向き HTTPS で直接呼ぶ。
- 機密情報は **AWS SSM Parameter Store**（SecureString）で管理。Secrets Manager は MVP では採用しない（自動ローテーションが不要、コスト $0 を優先）。
...
**Language/Version**: TypeScript（最新安定版、5.x または 6 系）/ Node.js 22.x（AWS Lambda の `nodejs22.x` ランタイム。Node.js 24 が AWS Lambda で正式 GA されていれば 24 に切り替える。`package.json` の `engines` と Terraform の `runtime` で一括管理）。
...
- **24/7 稼働**：Lambda は常時起動不要（オンデマンド）。Supabase は無料プランで 7 日無アクティブ Pause があるため EventBridge Scheduled Rule + keep-alive Lambda で 6 日に 1 回 SELECT 1 を発行する（FR-027）。
- **Page Access Token の長期化**：Meta Graph API で `fb_exchange_token` を介して長期トークン化し SSM Parameter Store に格納。
- **ユーザー認証**：Supabase Auth が JWT を発行し、アプリ側は HttpOnly Cookie で受け取る。DB にユーザーテーブル／セッションテーブルを持たない。
...
- LOC 目安：~2500 行（Worker Lambda + AI 呼び出し追加で +500 行）
...
| ゲート | 判定 | 根拠 |
|--------|------|------|
| **YAGNI（不要機能を入れない）** | ✅ PASS | 仕様の Out of Scope を厳守。AI 自動分類、Instagram DM、Slack 通知、顧客管理、商品管理、マルチテナント、トーン設定、AI 自動送信はすべて除外済み。AI 下書き生成のみ MVP に含む（プロダクトの目玉機能のため）。 |
...
│   │   ├── db/
│   │   │   ├── schema.ts         # Drizzle スキーマ（5 エンティティ：connected_pages, conversations, messages, ai_drafts, deletion_log）
│   │   │   ├── client.ts         # Supabase Pooler 接続
│   │   │   └── migrations/       # drizzle-kit が生成
│   │   ├── services/
│   │   │   ├── messenger.ts      # Send API ラッパー
│   │   │   ├── messenger.test.ts
│   │   │   ├── auth.ts           # Supabase Auth クライアント（signInWithPassword、signOut、JWT 検証）
│   │   │   ├── auth.test.ts
...
> Constitution Check で違反なし。SQS + Worker Lambda の追加は Meta 20 秒 SLA + AI レイテンシの物理制約による必然のため、複雑性正当化として明記済み（Constitution Check 表内）。本セクションは空。
