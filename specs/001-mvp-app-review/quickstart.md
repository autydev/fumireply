...
| Node.js 22.x（Lambda が 24 を GA したら 24）/ **npm**（pnpm・yarn は使わない。lockfile は `package-lock.json` を正とし、CI は `npm ci` でインストールする） | ローカル開発用 |
...
aws ssm put-parameter \
  ...
  --type "SecureString" --value "<LONG_PAGE_ACCESS_TOKEN>"
...
- **keep-alive-lambda**: Supabase Pause 回避 Lambda（EventBridge `rate(6 days)` Trigger）
...
### 3.1 Supabase Auth テストユーザー作成（Terraform apply 後）

Terraform で AWS リソースが作られた後、Supabase の Admin API でテストユーザーを作成する：
...
| **Supabase Pause 監視** | keep-alive Lambda 失敗を即時通知、Supabase ダッシュボードでも 1 日 1 回確認 |
| Page Access Token 失効 | 管理画面ヘッダーで検知 → 手動で再発行 → SSM 更新 → app-lambda 再デプロイ |
...
2. keep-alive Lambda の最終成功時刻を CloudWatch Metrics で確認
3. EventBridge Rule が有効か確認
4. 再発防止としてアラームの感度を上げる（`rate(3 days)` 等）
