...
| **`/fumireply/master-encryption-key`** | SecureString | **AES-256 マスター鍵（32 bytes hex）**。`connected_pages.page_access_token_encrypted` の暗号化に使用。**全テナントで共有**（テナント増加時の SSM 操作を不要にするため、env 名を含めない設計）|

**廃止したパラメータ**:
- `/fumireply/review/meta/page-access-token` — DB 暗号化カラムへ移行。テナントごとに増えるトークンを SSM で持つ運用は SaaS スケールに耐えないため。
...
| リソース | 設定 |
|----------|------|
| Lambda Function | `nodejs24.x`、メモリ 1024MB、タイムアウト 30 秒、**VPC 外配置**、Lambda Web Adapter Layer を attach（`arn:aws:lambda:ap-northeast-1:753240598075:layer:LambdaAdapterLayerX86:<latest>`）|
...
| リソース | 設定 |
|----------|------|
| Lambda Function | `nodejs24.x`、メモリ 512MB、タイムアウト 10 秒、**VPC 外**、Web Adapter なし（API Gateway イベント直接ハンドリング）|
...
| リソース | 設定 |
|----------|------|
| Lambda Function | `nodejs24.x`、メモリ 512MB、タイムアウト 60 秒、**VPC 外** |
...
**責務**: Supabase 無料プランの自動 Pause（7 日無アクティブで発動）を回避するため、**1 日 1 回**定期的に DB に SELECT 1 を発行する（FR-027）。Pause が発動すると Webhook 全失敗 → 審査差し戻しの致命的リスクのため、多重防御（高頻度 + 自動リトライ + 即時通知 + 連続失敗早期検知）を設計に組み込む。

| リソース | 設定 |
|----------|------|
| Lambda Function | `nodejs24.x`、メモリ 256MB、タイムアウト 30 秒、**VPC 外** |
| 環境変数 | `SSM_PATH_PREFIX=/fumireply/review/` |
| IAM Role | SSM 読み取り（DB URL 取得用）、CloudWatch Logs、SNS Publish（致命時の自己エスカレーション用、observability の SNS Topic ARN を環境変数で渡す）|
| EventBridge Scheduled Rule | **`rate(1 day)`**（毎日 09:00 JST 起動を推奨。Supabase の 7 日 Pause 閾値に対して 7 日のマージン）|
| EventBridge Target | Lambda Function（Permission 設定）+ Retry Policy（`maximum_retry_attempts = 2`、`maximum_event_age_in_seconds = 3600`）|
| Lambda 関数の OnFailure Destination | observability モジュールの SNS Topic（最終失敗時に直接通知配信）|
| Lambda 関数の `dead_letter_config` | observability の SNS Topic を指定（非同期失敗の最後の砦）|
| CloudWatch Alarm（observability モジュール側で定義）| (a) `Errors >= 1` を即時通知（DataPointsToAlarm=1）、(b) `Invocations < 1 in 36 hours` で「実行されていない」異常検知 |

**Lambda 内部のリトライ実装**:
- DB 接続〜`SELECT 1` で例外時、**指数バックオフ（500ms / 1.5s / 4.5s）で 3 回**リトライ
- 3 回失敗したら：
  1. CloudWatch Logs に `keepalive_critical` 構造化ログを出力
  2. SNS Publish で「Supabase keep-alive 失敗。手動で Supabase ダッシュボードを確認してください」を発報
  3. throw して Lambda を失敗終了 → EventBridge Retry が走る → それも失敗したら DLQ（SNS）へ流れる
- 多重に経路を確保：(i) Lambda 内部リトライ、(ii) EventBridge 自動リトライ、(iii) `OnFailure` Destination、(iv) CloudWatch Errors アラーム、(v) Invocations アラーム

**Inputs**:
- `name_prefix`, `tags`, `ssm_path_prefix`
- `sns_topic_arn`（observability モジュールから注入。失敗通知 + DLQ + OnFailure 用）
...
**設計上の注意**:
- **1 日 1 回**にすることで、1 日失敗しても 6 日間の猶予がある。猶予期間中に手動で Supabase Resume するか、原因調査・修正できる。
- アラーム通知先は SNS Topic 経由でメール + Slack（observability モジュール）。
- 監視メトリクスは observability モジュールに集約定義（後述 §3.8 のアラーム表に含める）。
...
| CloudWatch Alarm | (a) app-lambda Error Rate > 1%、(b) webhook-lambda Error Rate > 0.5%、(c) ai-worker DLQ Approximate Message Visible > 0、(d) ai-worker Duration p95 > 30s、**(e) keep-alive Errors >= 1（即時、DataPointsToAlarm=1）**、**(f) keep-alive Invocations < 1 in 36h**（実行されていない異常）、(g) keep-alive OnFailure Destination からの SNS Publish |
...
- 日次：Supabase keep-alive Lambda の Invocations（過去 24 時間に 1 回起動 + 成功）を CloudWatch で確認。Errors アラームが鳴ったら audit-runbook の対応手順を実行
- 月次：Meta Page Access Token の有効性確認（テナントごと、ローテーション不要だが念のため）。Phase 2 ではマスター鍵ローテーション運用を整備
...
| **セルフサインアップ + Stripe 課金**（Phase 2 主要機能）| `/signup` 画面 + tenant 作成 fn + Stripe Customer 作成 + Stripe Webhook Lambda（`modules/stripe-webhook-lambda` 新規）+ プラン管理 |
| **マスター鍵ローテーション運用**| 新鍵生成 → 全 `connected_pages.page_access_token_encrypted` を旧鍵で復号 → 新鍵で再暗号化 → SSM 更新。多鍵運用カラム `key_version` 追加で zero-downtime ローテーション |
| **マスター鍵を AWS KMS へ移行**| envelope encryption（KMS-encrypted DEK + per-row 暗号化）に移行する選択肢。コスト増だが監査要件が厳しい場合に有効 |
...
| マルチオペレーター対応（Phase 3+）| `tenant_users` 中間テーブル追加、ロールベース権限制御、招待フロー |
