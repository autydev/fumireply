...
**Tenancy**: マルチテナント。JWT の `user_metadata.tenant_id` クレームでテナントを識別（R-015）。

管理画面が呼び出す内部 API。全エンドポイント（`login` を除く）は JWT 認証ミドルウェアを通過する必須（FR-009）。未認証の場合は 401 で `/login` にリダイレクト。**全 DB 操作は `withTenant(tenant_id, fn)` トランザクションヘルパで囲み RLS を有効化する**（テナント越境を DB レイヤで防止）。
...
// 成功
{
  ...
  user: {
    ...
    email: string
    tenantId: string    // 所属テナント UUID（user_metadata.tenant_id）
...
login 成功後、middleware が **`tenants WHERE id = tenant_id AND status = 'active'` を確認**する。`status='suspended'` または `'deleted'` の場合は強制ログアウト + `/login?error=tenant_suspended` にリダイレクト。
...
1. JWT 認証確認 → tenantId 取得（ロール分岐は MVP では実装しない）
2. `withTenant(tenantId, async (tx) => { ... })` で以下を実行：
   - `conversations` から取得、`last_inbound_at` が 24h 以内か確認（超過なら `outside_window` エラー）
   - `connected_pages` から `page_access_token_encrypted` を取得 → `crypto.decrypt` で復号
   - `messages` に `send_status='pending'`, `sent_by_auth_uid=JWT.sub`, `tenant_id=tenantId` で INSERT
3. Meta Send API を呼び出し（[meta-send-api.md](./meta-send-api.md) 準拠）
4. 結果に応じて `withTenant` 内で `messages` UPDATE（`send_status='sent'/'failed'`、`conversations.last_message_at` 更新）
...
1. JWT 認証ミドルウェア通過 → tenantId 取得
2. `withTenant(tenantId, ...)` 内で `connected_pages` を SELECT（RLS により自テナントの行のみ）
3. `connected_pages.page_access_token_encrypted` を `crypto.decrypt(blob, masterKey)` で復号（マスター鍵は SSM から取得しメモリキャッシュ）
4. 直近で送信失敗（`send_error='token_expired'`）が記録されているかを `messages` から確認
5. 5 分のサーバーキャッシュ（tenantId 単位）で結果を返す
...
全 `serverFn`（`login` 除く）は以下のミドルウェアを通過する。JWT をステートレスに検証 + テナント有効性確認 + RLS 用 `app.tenant_id` 設定までを担う。
...
middleware: async ({ request }) => {
  ...
  if (!user) {
    user = await tryRefresh(request)
    if (!user) throw redirect({ to: '/login' })
  }

  // tenant_id を JWT から抽出
  const tenantId = user.user_metadata?.tenant_id as string | undefined
  if (!tenantId) {
    throw redirect({ to: '/login?error=no_tenant' })
  }

  // tenant ステータス確認（service role で1クエリ、RLS バイパス）
  ...
  return {
    ...
    // serverFn 本体は db アクセスを `withTenant(tenantId, async (tx) => { ... })` で囲む
  }
}
```
...
```ts
// app/src/routes/(app)/inbox/-lib/list-conversations.fn.ts
export const listConversationsFn = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .handler(async ({ context }) => {
    return withTenant(context.user.tenantId, async (tx) => {
      // この transaction 内で発行される全クエリは RLS により tenant_id でフィルタされる
      return tx.select().from(conversations).orderBy(desc(conversations.lastMessageAt))
    })
  })
```
