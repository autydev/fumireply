...
- [ ] T021 [P] [US2] Add tests "video / audio / file inbound の種別と保存" in `webhook/src/handler.test.ts`: それぞれ `messageType` 正 / S3 保存 / attachments 記録 / 空バブル要素なし (`body=''`)。sticker は DL なし (`fetch` 0 回) で `[{type:'sticker',s3Key:null}]`、未知 type (`fallback`) は `unknown` 記録を assert。video/audio/file でも **SQS enqueue (AI 下書き) が呼ばれない**ことを assert (FR-014)。
