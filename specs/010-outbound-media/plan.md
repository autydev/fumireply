...
- **2 通逐次送信 + 全体時間予算** (research D4): parts = [text?, image] を順に (pending INSERT → 送信 → 確定)。`SEND_TOTAL_BUDGET_MS = 22s` で 2 通目の pending 放置 (008 の教訓) を防ぐ。1 通目失敗でも 2 通目は独立試行
...
**Testing & CI**:
- vitest (app): `create-upload-url.server` (allowlist/サイズ/ウィンドウ/未設定環境/キー採番形式) / `send-reply.server` 拡張 (s3Key 検証・HeadObject 検証・2 通逐次と順序・部分失敗・時間予算・echo claim 回復がパーツ単位で働く・attachment 省略時の完全後方互換) / `messenger` の画像ペイロード形状とエラーマッピング共用
- vitest (app, 統合): 既存 `send-reply` 統合テストのモック DB を拡張し 2 行 INSERT・attachments JSONB 形状を検証
- Playwright E2E スモーク: 添付ボタン表示 (seed で mediaUploadEnabled) と client バリデーションエラー表示
...
- 画像送信 1 通あたりの Meta 呼び出しは従来のテキスト送信と同一プロファイル (5s timeout × 最大 3 試行)。presigned GET/PUT の発行はローカル署名 <1ms

**Constraints**:
- **Lambda 30s と 2 通送信**: 最悪ケース (両パーツがフルリトライ) は ~43s で timeout を超えるため、全体時間予算 22s + 残余 5s 未満で 2 通目を `failed('timeout')` 即確定 (pending 放置ゼロ — 008 D2 と同じ思想)
- **サイズ・型の強制点**: client (UX) → presigned PUT の ContentType 署名 (型) → 送信時 HeadObject (型 + サイズの最終強制)。presigned PUT 単体ではサイズを強制できないことを前提にした設計 (research D1)
...
- 追加・変更コード LOC 目安: ~600 行
  - `app/src/server/services/media-upload.ts` (NEW: 定数 + presigned PUT 発行 + HeadObject 検証 + キー採番/検証) ~80 行
  - `app/src/routes/(app)/threads/$id/-lib/create-upload-url.fn.ts` + `.server.ts` (NEW) ~70 行
  - `app/src/routes/(app)/threads/$id/-lib/send-reply.fn.ts` + `send-reply.server.ts` (MODIFY: attachment 入力 + parts 逐次送信 + 時間予算) ~110 行
  - `app/src/server/services/messenger.ts` (MODIFY: 画像 attachment ペイロード) ~25 行
...
**Structure Decision**: 書き込み系ユーティリティを `media-upload.ts` に新設し、009 の読み取り系 `media-url.ts` と対称に保つ (webhook 側 `media.ts` = 受信書き込み、app 側 upload = 送信書き込み)。送信編成の複雑化 (2 通逐次 + 時間予算) は `.server.ts` 純ロジック層に閉じ込め、server fn (`send-reply.fn.ts`) は入力検証と委譲に徹する。webhook / ai-worker / ThreadMessages に手を入れないことで、変更面を「送信経路 + 返信フォーム + インフラ 2 点」に限定する。
