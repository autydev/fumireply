...
2. **形式別の送信確認**: png / gif / **webp** をそれぞれ送信し顧客側で表示されること。**webp が Meta 側で拒否される場合は `ALLOWED_IMAGE_TYPES` から除外して再デプロイ** (research D5)
3. **テキスト+画像** (US2): 両方入力して送信 → 顧客側にテキスト → 画像の順で 2 通 / 自社スレッドにも 2 メッセージ
4. **バリデーション** (US3): pdf 選択 → 選択時エラー / 26MB 画像 → 選択時エラー / (devtools で client 検証を迂回した場合もサーバーが拒否)
