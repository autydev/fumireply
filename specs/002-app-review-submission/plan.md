...
- **Connect Facebook Page UI**: 既存 SSR Lambda（TanStack Start）に新規ルート `/onboarding/connect-page` を追加。Lambda 構成・SQS・AI Worker は変更しない。Facebook JS SDK をクライアント側で動的ロードし、Facebook Login for Business（`config_id` で 4 権限を束ねた Login Configuration）のポップアップで同意取得 → server fn に短期 user token を POST → サーバー側で `fb_exchange_token` → 長期 user token を暗号化して httpOnly Cookie `fb_connect_session`（10 分）に退避 → ユーザーが接続対象の数値 **Page ID を手入力** → server fn が Cookie の user token で `GET /{page-id}`（`fetchPageWithToken`）から正式名称＋長期 Page Access Token をサーバ側取得 → `POST /{page-id}/subscribed_apps` + 既存 `crypto.ts` で AES-256-GCM 暗号化 + `connected_pages` UPSERT。Page Access Token / user token はブラウザに一切送出しない。
...
**HTTP クライアント方針**: 001 から継続。グローバル `fetch` のみを使用。`axios` 系の新規導入は禁止。Facebook Graph API（`fb_exchange_token` / `GET /{page-id}` / `/{page-id}/subscribed_apps`）への呼び出しはすべて server fn 内で `fetch` + `AbortSignal.timeout(ms)` で実装する。
...
**Performance Goals**:
- Page 接続フロー全体（FB Login ポップアップ → Page ID 入力 → DB 保存 → /inbox 遷移）p95 < 30 秒、SC-003 の「初回 5 分以内」を確実に達成
- Facebook Graph API 呼び出し（fb_exchange_token / GET /{page-id} / /{page-id}/subscribed_apps）p95 < 3 秒（タイムアウト 10 秒）
...
| **観測性** | ✅ PASS | Connect Page server fn は構造化ログで「token_exchange / fetch_page (GET /{page-id}) / subscribe_apps / encrypt / db_upsert」の各段階を記録。Graph API レスポンスのエラーコード（OAuthException 等）を拾って CloudWatch に出す。（2026-05-16 再チェック: 全 6 ゲート PASS 維持。実装はテスト可能性ゲートを実証 — 178 tests green、ハンドラ抽出は既存 `handleSendReply`/`performSetLocale` 慣習に準拠し新規抽象なし）|
...
│   ├── facebook-graph.md       # fb_exchange_token, GET /{page-id} (fetchPageWithToken), /{page-id}/subscribed_apps
...
│   │   │           ├── index.tsx                          # NEW: ルート本体
│   │   │           ├── -components/
│   │   │           │   ├── ConnectFacebookButton.tsx     # NEW: FB JS SDK 呼び出し（config_id Login for Business）
│   │   │           │   ├── PageIdInput.tsx               # NEW: 数値 Page ID 手入力フォーム
│   │   │           │   └── ConnectErrorPanel.tsx         # NEW: エラー表示・再試行
│   │   │           └── -lib/
│   │   │               ├── exchange-and-list.fn.ts       # NEW: server fn: 短期 token → 長期 user token → 暗号化 httpOnly Cookie 退避
│   │   │               ├── connect-page.fn.ts            # NEW: server fn: pageId → fetchPageWithToken → subscribe + AES + UPSERT
│   │   │               └── check-connected-pages.fn.ts   # NEW: server fn: 接続済 active 件数（forward/reverse guard 共用）
│   │   ├── (auth)/login/
│   │   │   └── -components/LoginForm.tsx    # MODIFY: i18n 化
│   │   └── ... (inbox, threads, public は既存)
│   ├── server/
│   │   └── services/
│   │       └── facebook.ts                  # NEW: Graph API ラッパー（fb_exchange_token / fetchPageWithToken / subscribed_apps; listPages は残置・未使用）
│   ├── lib/
│   │   ├── i18n/                            # NEW
