...
- [x] T028 [US2] `app/src/routes/(auth)/login/-components/LoginForm.tsx`: replace `Email`/`Password` labels, submit button, error messages with `m.login_email_label()` / `m.login_password_label()` / `m.login_submit_button()` / `m.login_error_invalid_credentials()` / `m.login_error_generic()` / `m.login_loading()`
- [x] T029 [US2] `app/src/routes/(app)/inbox/-components/InboxList.tsx`: replace filter labels (`すべて`/`未読`/`期限超過`), empty state, 24h window badge with `m.inbox_filter_all()` / `m.inbox_filter_unread()` / `m.inbox_filter_overdue()` / `m.inbox_empty_state()` / `m.thread_window_outside_24h()`
- [x] T030 [US2] `app/src/routes/(app)/threads/$id/-components/ThreadMessages.tsx` + `index.tsx`: replace `送信中…` and `24h窓内`/`24h窓外` badge strings with `m.reply_sending_button()` / `m.thread_window_within_24h()` / `m.thread_window_outside_24h()`
- [x] T031 [US2] `app/src/routes/(app)/threads/$id/-components/ReplyForm.tsx`: replace all JA strings (placeholder, buttons, banners, errors, aria-label, policy countdown) with `m.reply_placeholder()` / `m.reply_send_button()` / `m.reply_sending_button()` / `m.reply_ai_suggestion_label()` / `m.reply_draft_saved()` / `m.reply_window_closed_warning()` / `m.reply_policy_countdown()` / `m.reply_policy_time_remaining({hours, minutes})` / `m.reply_error_*()` / `m.reply_saving()` / `m.reply_body_label()`
- [x] T032 [US2] `app/src/routes/(app)/threads/$id/-components/DraftBanner.tsx`: add `reply_draft_generating` key to `app/messages/en.json` + `app/messages/ja.json`, run `paraglide-js compile`, replace `下書き生成中…` with `m.reply_draft_generating()`
