...
**Scale/Scope**: 変更対象は `ai-worker/src/handler.ts` + ai-worker テスト 2 ファイル + app 統合テスト 2 ファイル(handler を独自 DB モックで駆動しているためクエリ形状に追随)+ ドキュメント。DB スキーマ・app プロダクションコード・webhook・terraform は変更なし
...
app/
└── tests/integration/
    ├── ai-draft-worker.test.ts                    # [変更] handler 駆動モックのクエリ形状追随
    └── ai-draft-uses-conversation-settings.test.ts # [変更] 同上
terraform/
└── modules/
...
**Structure Decision**: 既存 ai-worker ワーカーへの最小修正。プロダクションコードの変更は `handler.ts` 1 ファイルに閉じ、インフラ(terraform)・app クライアント・DB スキーマは一切変更しない。ただし app の統合テスト 2 ファイルは ai-worker handler を独自 DB モックで駆動しているため、境界クエリの形状変更に追随が必要(実装時に CI で検出)。app 側 `DraftBanner` は `error` 文字列を透過表示するため、新エラーコード `internal_error` の追加にクライアント変更は不要。
