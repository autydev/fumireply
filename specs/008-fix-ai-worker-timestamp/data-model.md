...
- **INV-1**: ジョブが SQS から消える(正常終了 or DLQ 行き)時点で、対応する draft は `pending` のままではない — ただし DLQ 行き(終端書き込み失敗、または tenant 解決失敗 — いずれも DB 断相当)のときのみ draft の状態は保証できない(既存挙動から悪化はしない)
