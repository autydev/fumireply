...
## 2026-06-27 以降の必須設定: `message_echoes` 購読

`specs/006-message-echoes-ingest/` で導入した「外部アプリ送信の取り込み」を機能させるため、Meta App 管理画面の Webhook 購読フィールドに **`message_echoes`** を追加する。

未購読の場合: 運営者が Messenger 公式アプリ等の **fumireply 以外のアプリ** から顧客に返信した送信が fumireply のスレッドに反映されず、スレッドが歯抜けになる。AI 下書きも「既に外部で返信済み」を認識できないため、未返信バッチ判定 (#004) の境界がズレる。

### 切替手順 (1 回限り、リリース時)

1. https://developers.facebook.com/apps/ で本番 App を開く → 「Messenger」→「Settings」→ 「Webhooks」の Edit
2. **Subscription Fields** ダイアログで `message_echoes` のチェックを ON にする (既存の `messages`, `messaging_postbacks` はそのまま)
3. 「Save」を押す → 直後に CloudWatch Logs (`/aws/lambda/<webhook-fn>`) で `external_echo_ingested` / `self_echo_confirmed` が出始めることを確認

### 緊急ロールバック

問題が起きたら同画面で `message_echoes` を OFF にする → Meta から echo 配信が止まり、新コードパスは未実行になる。コードロールバックは不要。

詳細: `specs/006-message-echoes-ingest/quickstart.md` §2 / §6 (トラブルシューティング)
